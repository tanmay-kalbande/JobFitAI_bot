...
const STRENGTH_ORDER: Record<ProofMapItem['strength'], number> = { gap: 0, moderate: 1, strong: 2 };

function sortProofMap(items: ProofMapItem[]): ProofMapItem[] {
    return [...items].sort((a, b) => STRENGTH_ORDER[a.strength] - STRENGTH_ORDER[b.strength]);
}

export function ProofMapPanel({ version, onClose, fullView = false }: ProofMapPanelProps) {
    const proofMap = version.proofMap || [];
    const sortedProofMap = sortProofMap(proofMap);
        ...
        <div className={`proof-map-panel ${fullView ? 'proof-map-panel-full' : ''}`}>
            ...
            <div className="proof-map-list">
                {proofMap.length === 0 ? (
                    ...
                    sortedProofMap.map((item, index) => (
                        <div key={`${item.requirement}-${index}`} className={`proof-map-card ${item.strength}`}>
                            <div className="proof-map-top">
                                <span className={`proof-map-badge ${item.strength}`}>
                                    {item.strength === 'gap' ? '⚠ Gap' : item.strength === 'moderate' ? '◑ Moderate' : '✓ Strong'}
                                </span>
