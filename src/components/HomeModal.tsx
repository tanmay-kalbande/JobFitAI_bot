...
export function HomeModal({ versions, onClose, onSelectVersion, onDeleteVersion, currentVersionId = null, mode = 'modal' }: HomeModalProps) {
    ...
    const filteredVersions = useMemo(() => {
        ...
        return list.filter(v =>
            ...
            (v.jobDescription ?? '').toLowerCase().includes(q) ||
            (v.model ?? '').toLowerCase().includes(q)
        );
    }, [activeCompany, groupedVersions, searchQuery]);

    const typeLabel = (type: ResumeVersion['type']) => {
        if (type === 'tailored') return { icon: 'T', color: '#3b9eff', label: 'Tailored' };
        if (type === 'cover-letter') return { icon: 'CL', color: '#c26b2d', label: 'Cover Letter' };
        if (type === 'cv') return { icon: 'CV', color: '#0f766e', label: 'CV' };
        if (type === 'fixed') return { icon: 'F', color: '#22c55e', label: 'Fixed' };
        return { icon: 'R', color: '#a78bfa', label: 'Resume' };
    };
    ...
    const getJobDescriptionPreview = (version: ResumeVersion) => {
        const text = version.jobDescription?.replace(/\s+/g, ' ').trim();
        if (!text) return '';
                ...
                <>
                    <div className="hm-search-bar">
                        ...
                        <input
                            ...
                            type="text"
                            placeholder="Search title, company, JD, or model..."
                            value={searchQuery}
                            ...
                            <div className="hm-list">
                                {filteredVersions.length === 0 ? (
                                    ...
                                    filteredVersions.map(version => {
                                        ...
                                        const jdPreview = getJobDescriptionPreview(version);
                                        const isActive = currentVersionId === version.id;
                                        return (
                                            <div
                                                key={version.id}
                                                className={`hm-resume-row ${isActive ? 'active' : ''}`}
                                                onClick={() => { onSelectVersion(version); onClose(); }}
                                            >
                                                <span className="hm-rr-icon" style={{ color: t.color, borderColor: `${t.color}66` }}>{t.icon}</span>
                                                <div className="hm-rr-body">
                                                    ...
                                                    {jdPreview && (
                                                        <p className="hm-rr-jd">{jdPreview}</p>
                                                    )}
                                                </div>
                                                {onDeleteVersion && (
                                                    <button
